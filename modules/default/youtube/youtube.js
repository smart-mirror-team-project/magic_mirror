Module.register("youtube", {
	defaults: {
		...
		apiKey: config.key, //??
		apiSecret: "",
		apiBase: "https://www.googleapis.com/youtube/v3/search?", // TODO: this should not be part of the weather.js file, but should be contained in the openweatherprovider
	...
	start: function () {
		...
		// ******************************************
		this.config.onStartPlay.id = "j5oS55XY-SI";
	},
	...
	loadVideo: function (payload) {
		...
		var fn = "cue" + method;
		this.videoLoop = payload.hasOwnProperty("loop") ? payload.loop : this.config.defaultLoop;
		this.videoShuffle = payload.hasOwnProperty("shuffle") ? payload.shuffle : this.config.defaultShuffle;
		this.videoAutoplay = payload.hasOwnProperty("autoplay") ? payload.autoplay : this.config.defaultAutoplay;
